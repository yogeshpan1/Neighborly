			...
			<div class="panel">
				<c:choose>
					<c:when test="${empty contactList}">
						<p style="color: #94a3b8; text-align: center; padding: 32px 0;">No contact messages yet.</p>
					</c:when>
					<c:otherwise>
						<c:forEach var="c" items="${contactList}">
							<div class="newsRow">
								...
								<div class="newsInfo">
									<h4><c:out value="${c.subject}" /></h4>
									<div class="newsMetaRow">
										<span class="newsCategoryBadge normal"><c:out value="${c.fullName}" /></span>
										<p><c:out value="${c.email}" /></p>
										<p>${c.createdAt}</p>
									</div>
								...
								<div class="newsActions">
									<form action="${pageContext.request.contextPath}/contactmessage"
										method="POST">
